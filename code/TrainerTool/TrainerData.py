def create_parameter(value, offset_suffix):
    DEFAULT_OFFSETS = [0x70, 0x18, 0x170, 0x98, 0x48, 0x10]
    offset = DEFAULT_OFFSETS + [offset_suffix, 0x00]
    
    return {'value': value, 'address': 0x0314EE60, 'offset': offset}
    

def create_upgrade_parameter(value):
    DEFAULT_OFFSETS = [0x48,0x10,0x8730, 0x00]
    
    return {'value': value, 'address': 0x0314EC90, 'offset': DEFAULT_OFFSETS}
                       

        
def infinite_currency():  # 無限貨幣
...
def unlock_all_achievements():  # 全成就解鎖
...
def unlock_all_characters_and_costumes():  # 全角色與服裝
...
def unlock_all_furniture():  # 全小屋家具
...
def infinite_house_items():  # 無限小屋道具
...
def get_trainer_data():
    ...
    functions = [
        create_parameter(99999.0, 0x9C0),  # God Mode
        create_parameter(99999.0, 0x1400),  # Infinite Attraction Range
        create_parameter(99999.0, 0x1E50),  # One-Hit Kill
        create_parameter(999.0, 0x7C0),  # Super High Attack Speed
        create_parameter(100.0, 0x1590),  # Infinite Skills
        create_upgrade_parameter(1.0),  # Infinite Upgrades
        create_upgrade_parameter(999.0),  # Stop Upgrading
        infinite_currency(),  # Infinite Currency
        unlock_all_weapons(),  # Unlock All Weapons
        unlock_all_achievements(),  # Unlock All Achievements
        unlock_all_characters_and_costumes(),  # Unlock All Characters and Costumes
        unlock_all_levels(),  # Unlock All Levels
        max_passive_skills(),  # Max Passive Skills
        unlock_all_furniture(),  # Unlock All Furniture
        infinite_house_items()  # Infinite House Items
    ]
