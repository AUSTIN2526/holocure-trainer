def dynamic_function():
    def create_parameter(value, offset_suffix):
        ...
        DEFAULT_VALUES = {'address': 0x0314EE60, 'interlock': False}

        offset = DEFAULT_OFFSETS + [offset_suffix, 0x00]
        return {'value': value, 'address': DEFAULT_VALUES['address'], 'offset': offset, 'interlock': DEFAULT_VALUES['interlock']}
    
    return (
        create_parameter(99999.0, 0x9C0),    # HP
        create_parameter(99999.0, 0x1400),   # Pick Range
        create_parameter(99999.0, 0x1E50),   # ATK
        create_parameter(999.0, 0x7C0),      # Haste
        create_parameter(100.0, 0x1590)       # EX
    )

def upgrade_effects():
    def create_parameter(value):
        DEFAULT_OFFSETS = [0x48,0x10,0x8730, 0x00]
        DEFAULT_VALUES = {'address': 0x0314EC90, 'interlock': True}
        return {'value': value, 'address': DEFAULT_VALUES['address'], 'offset': DEFAULT_OFFSETS, 'interlock': DEFAULT_VALUES['interlock']}
                       
    return (
        create_parameter(1.0),     # Level up
        create_parameter(999.0)    # Stop Level up                           
    )
...
def save_editor_info():
    ...
    default_values = {
        'holoCoins': MAX_HOLO_COINS,
        'randomMoneyKey': 0,
        'fishSand': MAX_HOLO_COINS,
        'tears': [[name, MAX_TEARS] for name in tears],
    }

    unlocked_items = {'unlockedItems': items, 'unlockedWeapons': weapons, 'seenCollabs': collabs}

    achievements_data = {'achievements': {name: achievements_format for name in achievements}}
